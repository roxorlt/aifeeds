...
- [ ] **9/8 旧审核链接打开 latest 整页刷新死循环**：审核链接钉在 `/aifeeds/latest/`，隔天打开变成「新一天页面 + 旧一天批次」，进度决策拿跨日期版本号比出「有新版」反复 reload（07:02–07:05 同一地址 17 次）。面板 commit 5214032 加日期护栏（847 测试通过），render release `e8c9f780…` tar 已在渲染机 `/tmp`；**launcher 切换待 owner 自行执行切换脚本**（脚本路径与步骤见 operations.md 9/8 节，本地文件），下一次出片生效。后续待拍板：审核链接改钉日期 `/aifeeds/<MMDD>/`（云端 `reviewLink` 两处 + 面板三处 pathname 校验）
