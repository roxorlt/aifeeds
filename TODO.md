...
### A12. 手机端重选后「确认并重新生成」没反应（2026-09-17，已修，次日页面生效）

**症状**：owner 在微信里重选内容后点底栏按钮，页面停在「重新生成进度：等待提交」；服务器 06:26–06:52 没收到任何提交请求，刷新后重提才成功。

**根因**：手机底栏 `#mobile-primary-cta` 的可点状态只在首屏和每 5 秒状态轮询时从审核区按钮抄一次，改选/重排不同步；改完立刻点（0–5 秒内）点的是灰按钮。进度框的 `hidden` 被 `.news-review-progress{display:grid}` 压掉，未提交就露出静态文案「等待提交」，看起来像提交了卡住。

- [x] 面板分支 `cc/20260902-review-ux-and-baton-race` fe6a710 + 7c56c4d：改选、提交中、提交失败都同步底栏；`[hidden]` 规则生效，静态文案改「已提交，正在等待渲染端回执…」；非本页发起的生成在决策区显示「当天视频正在生成 · N%」。4 条新测试，全套 858/857/1 skip。
- [x] render release 已切换（9/17 07:20），9/18 06:00 页面起生效。
- [ ] 底栏点击处理判断 `body.dataset.pageState`，底栏文案判断 `bar.dataset.state`，两个真相来源，将来改状态机时收成一个。
- [ ] 进度框可见性由两个函数分管（有生成目标归 `refreshNewsReviewProgress`，无目标归 `applyPageState`），以 `reviewGenerationTarget` 为界；若日后别处清空该变量会跳变，届时收回一处。
...
- [ ] **9/8 旧审核链接打开 latest 整页刷新死循环**：审核链接钉在 `/aifeeds/latest/`，隔天打开变成「新一天页面 + 旧一天批次」，进度决策拿跨日期版本号比出「有新版」反复 reload（07:02–07:05 同一地址 17 次）。面板 commit 5214032 加日期护栏；同日 acafb71 让审核区默认载入今天、展开「选题决策」/「重新调整」自动切到当日、「打开今日」按下即把日历置为今天（owner 反馈「每次都要在日历勾选今日再打开」；854 测试通过）。两者合并为 render release `5e43515e…`，9/8 08:15 已切换 launcher，下一次出片（9/9 06:00 或当天重做）生效；当天旧页面仍循环，08:27 面板机 nginx 加应急规则「latest 带非今天 review_date 一律 302 回今天」（细节与备份见 operations.md 9/8 节）。验收：从微信历史打开旧链接应直接显示当天候选、日历为当天、不再自动刷新。后续待拍板：审核链接改钉日期 `/aifeeds/<MMDD>/`（云端 `reviewLink` 两处 + 面板三处 pathname 校验）
