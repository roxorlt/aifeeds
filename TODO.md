...
- [x] 面板：审核确认失败中文提示（78c3c3b）+ 审核页自动跟到最新批次、补录入池后自动刷新候选区（ac7f39f、a7ef2ad）已随 render release `b7ab159e…` 上线（9/7 10:32 切换 launcher，下一次出片生效）；代理超时 85s 已装进面板树，仍需重启 `aifeeds-x-card` 生效
