...
//   - root-relative "/foo" → github.com/<owner>/<repo>/blob/<branch>/foo
//   - absolute http/https/data: untouched
...
// Tailwind-styled markdown elements + relative-URL rewriter for owner/repo/branch.
...
export function GithubDrawerBody({ item }: Props) {
  ...
  // Markdown components rebuilt per repo so img/a can resolve relative URLs
  // against the correct GitHub repo + branch.
  const defaultBranch = (extra.default_branch as string | undefined) || "main";
  const markdownComponents = makeMarkdownComponents(owner, ownerRepo.split("/")[1] || "", defaultBranch);
