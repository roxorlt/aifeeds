...
> **目标**：用 `ai-feeds.cc`（已备案，主体「科赞源信」、对外品牌「AI源信」）做国内 SEO / GEO 的合规入口。`.com` 继续承载完整动态产品，`.cc` 只做「从 `.com` 数据库抓取 + 渲染成静态页」的内容矩阵，把 C 端用户引流回 `.com` 使用完整功能。
>
...
| `.cc` 备案状态 | 已备案，主体「科赞源信」 | 国内合规入口的物质基础已就绪 |
