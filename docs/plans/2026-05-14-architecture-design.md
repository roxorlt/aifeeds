...
> **⚠️ 快照说明（2026-05-15 补）**：本文档是 2026-05-14 commit `c26a712` 时点的架构现状分析。main 后续合入 PR #11（cron PushDeer 推送）/ PR #12（PH lazy-enrich）/ PR #14（PH scraper 退役 + 全切 worker cron）/ PR #16（D1 backup Workflow）等多个改动，文档部分结论已被覆盖 —— 尤其「业务级告警 0 覆盖」「数据备份 launchd 缺」这两条已部分或完全解决。阅读时对照最新 [TODO.md](../../TODO.md) 和 `git log --since=2026-05-14` 看实际状态。
