...
`proof_excerpt_v1` 的冻结字段为：`contract='proof_excerpt_v1'`、`algorithm='utf8-nfc-ws1-codepoint-prefix-v1'`、`max_code_points=3000`、`sha256`（64 位小写 hex）、`utf8_bytes`、`code_points`。字段集合必须 byte-exact；legacy `max`、错误 `max_code_points` 或同时出现两者均失败关闭。`response_hmac_contract='hmac-sha256-canonical-json-all-fields-except-response_hmac-v1'`；response HMAC 覆盖 audit 中除 `response_hmac` 外的全部字段，header 不携带 excerpt 文本。CF/HK 共同 golden artifact 固定为 `workflows/aifeeds-daily/fixtures/proof-excerpt-v1-golden.json`，canonical file SHA-256 固定为 `213f5b82e0e89d6c66b7c41d7d44824eea77196b5ee032c8b02e971adffb5a4c`；任一仓内容或 checksum 漂移都阻断。
