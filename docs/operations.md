...
- `manual-news-lead-workflow` 负责安全取证、严格 JSON 事实核验、事件聚类/跨日去重和评分；timeout、429、5xx、D1、研究网关与模型瞬时错误由 Workflow durable step 重试。每次处理持有 owner/attempt/lease，重试耗尽后仅 owner 可 CAS 为 failed；过期的中间状态会重置为 validating 并重新入队。Workflow binding 缺失返回 503，创建实例失败写入可见 failed 状态，绝不回落到请求生命周期。生产与 staging 使用独立 Workflow binding `MANUAL_NEWS_LEAD_WORKFLOW`。
