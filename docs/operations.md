...
最后更新：2026-08-13（手工补录行业新闻线索已全量上线 prod：PR #212 合并 + CI 部署，D1 migration 033-036 已执行，4 个 CF 配置/secret（MANUAL_NEWS_RESEARCH_ORIGIN / MANUAL_NEWS_RESEARCH_TOKEN / MANUAL_NEWS_RESEARCH_RESPONSE_SECRET / MANUAL_NEWS_VERIFICATION_SECRET）已纳入环境清单。证据消费升级为 signed `article_text_v2` + proof policy v10；发布必须先完成 HK 网关、sandbox、response secret 与 legacy/v1/v2 smoke，再进入 CF staging 和 production。见「手工补录行业新闻线索」）
...
- 内部 API（`Bearer DAILY_NEWS_REVIEW_SECRET`）：
  ...
  - `GET /api/digest/daily-news-leads?date=YYYY-MM-DD`：最多返回 50 条 bounded summary（含 `evidence_count`，不含 evidence/audit/excerpt）；`GET /api/digest/daily-news-leads/:id`：按需返回单条 detail，最多 8 条、每条最多 3000 Unicode code points / 12000 UTF-8 bytes 的 excerpt，不返回完整正文或 fetch audit。工作台必须先 list，再按需 GET detail。
...
- migration `033-manual-news-leads.sql` 新增 `manual_news_leads`、`manual_news_evidence`、`manual_news_event_assessments`、`manual_news_lead_audit` 和 date/lineage 单调 `daily_news_review_candidate_generations`，证据行以 `fetch_audit_json` 保存规范化抓取/提取审计；同时给 `daily_news_review_batches` 增加不可变 revision / supersedes / origin / lineage / current / candidate generation 字段及“每日期/lineage 仅一个 active revision”的部分唯一索引。既有日期和 batch 从 generation 0 懒初始化，部署迁移不要求回填业务数据。additive migration `034-manual-news-assessment-verifications.sql` 新增独立 verification 历史表、每 lead 仅一个 active 的部分唯一索引及 active/history 查询索引；`verification_json` 保存经严格校验的逐事实 quote 结果。不得修改或重写已执行的 033。
- `MANUAL_NEWS_VERIFICATION_SECRET` 是每环境独立、恰好 64 个小写十六进制字符的 Wrangler secret（建议用密码学安全随机源生成 32 bytes 后编码为 lowercase hex）。部署前分别执行 `wrangler secret put MANUAL_NEWS_VERIFICATION_SECRET` 和 `wrangler secret put MANUAL_NEWS_VERIFICATION_SECRET --env staging`，不要写入 `wrangler.toml` 或仓库。提交与 retry 在入队前检查格式，缺失/非法返回 503；GET 仍可读取 lead，但隐藏 assessment。系统以它对 lead/version、完整最终 assessment（含 `source_atomic_facts_v2` 源语言原子事实、稳定 fact ID、`zh_editorial_projection_v2` 中文编辑投影及逐句映射）、本地确定性派生的双语语义槽（来源归因、认识可能性、计划/未来、进行/完成和 polarity 分组正交签名；参与者/数量范围、对象关系与否定、与具体目标产品绑定的后缀、实体、地区、原因、版本及绝对/相对时间），以及 `consumed-semantic-spans-v1` predicate/object 残留门控、完整 evidence、规范化 signed-v2 provenance 和完整 validated `verification_json` 做 HMAC-SHA256。当前验签 policy 为 `fact-evidence-projection-hmac-v10`；v10 proof 只允许每条 evidence 都带完整、结构一致的 `article_text_v2` provenance。v9、旧 policy、legacy/v1/unsigned/malformed evidence、错误摘要/HMAC 或 invalidated proof 一律 fail closed，经现有 invalid-proof 路径隐藏并隔离，必须显式 retry/re-evidence；绝不原地迁移或升级 proof。lead API、重放、持久化加载、确认、候选重建、冻结/finalize 与跨日 prior-event 加载统一复算同一 current-proof 条件。已持久化 evidence 的检查不使用当前墙钟重新判定采集新鲜度；采集时间窗只在 transport 接收时执行。
...
- Worker 不直接 fetch 用户域名；URL 取证和开放网络搜索只调用 `MANUAL_NEWS_RESEARCH_ORIGIN` 固定 HTTPS 研究网关，并使用 secret `MANUAL_NEWS_RESEARCH_TOKEN`。CF secret inventory 还必须包含每环境独立的 `MANUAL_NEWS_RESEARCH_RESPONSE_SECRET`，与对应 HK `AIFEEDS_MANUAL_NEWS_RESPONSE_SECRET` 使用同一 32-byte lowercase-hex key。正式 `/v1/document` 请求显式携带 `extraction_mode=article_text_v2`、`response_profile=proof_excerpt_v1`、随机 nonce、canonical request timestamp、limits 与 max redirects；缺失或未知 profile 一律失败关闭，旧 v2 兼容仅由 HK 承担。响应 audit 必须完整包含 `protocol_version=article_text_v2`、相同 nonce/timestamp、canonical extracted timestamp、逐跳 peer audit、final URL、source/extraction、requested/applied/actual limits、无截断标记、成功 parser、完整 body SHA-256、`response_hmac_contract=hmac-sha256-canonical-json-all-fields-except-response_hmac-v1`、严格 `proof_excerpt` claims 及对除 `response_hmac` 外完整 canonical audit 的 HMAC。HTML/XHTML 仅接受 Chromium 提取的完整单一 `article_text`，并要求 title/published_at/selection/content_complete document metadata 一致；禁止 CF 重解释 raw HTML。text/plain、JSON、PDF 分别保留 `text`、`json`、`pdf_text` 语义，但也必须处于同一 signed profile envelope。所有模式都先认证 response HMAC，再以 audit body SHA-256 和 actual byte/code-point sizes 验证完整 transport body；随后 CF 按 `utf8-nfc-ws1-codepoint-prefix-v1` 独立派生最多 3000 code points / 12000 UTF-8 bytes、无省略号的 excerpt，并立即解除完整 body 引用。完整 body 不得进入 evidence、claims、D1、Workflow step output、API DTO、日志/错误 metadata 或 proof canonical payload；仅 bounded excerpt 与 signed audit/provenance 持久化。current 校验只复验 response HMAC 及 persisted excerpt 对 signed digest/sizes 的一致性，不使用当前墙钟。缺失 profile、任意替换 excerpt、超限、第 9 条、重复 evidence ID/final URL 或把完整 body 塞入 claims 均失败关闭，不回退 legacy/v1。跨仓 golden artifact 为 `workflows/aifeeds-daily/fixtures/proof-excerpt-v1-golden.json`，固定 SHA-256 `c576f24f0e1c22b8b828c3bbd0bd7157721768197aa279a911050e475bb87c8a`；显式 whitespace 集包含 U+0085，CF/HK 任一漂移即阻断。
- 发布顺序：先把 HK feature 合入 HK 主线，安装并验证 dedicated Chromium sandbox/account/resource gate、研究网关和 `AIFEEDS_MANUAL_NEWS_RESPONSE_SECRET`；随后在 HK 完成 authenticated gateway/secret 以及 legacy、v1、v2、tamper/stale/private-target smoke。以上全部通过后，才配置/部署 CF staging（含 `MANUAL_NEWS_RESEARCH_RESPONSE_SECRET`、migration/Workflow）并做端到端 smoke；最后才进入 CF production。禁止 consumer-first 发布，也不得在 HK v2 尚未可用时启用 CF v10 enforcement。
