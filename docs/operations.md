...
- Worker 不直接 fetch 用户域名；URL 取证和开放网络搜索只调用 `MANUAL_NEWS_RESEARCH_ORIGIN` 固定 HTTPS 研究网关，并使用 secret `MANUAL_NEWS_RESEARCH_TOKEN`。CF secret inventory 还必须包含每环境独立的 `MANUAL_NEWS_RESEARCH_RESPONSE_SECRET`，与对应 HK `AIFEEDS_MANUAL_NEWS_RESPONSE_SECRET` 使用同一 32-byte lowercase-hex key。正式 `/v1/document` 请求携带 `extraction_mode=article_text_v2`、随机 nonce、canonical request timestamp、limits 与 max redirects；响应 audit 必须完整包含 `protocol_version=article_text_v2`、相同 nonce/timestamp、canonical extracted timestamp、逐跳 peer audit、final URL、source/extraction、requested/applied/actual limits、无截断标记、成功 parser、body SHA-256 及对除 `response_hmac` 外完整 canonical audit 的 HMAC。HTML/XHTML 仅接受 Chromium 提取的完整单一 `article_text`，并要求 title/published_at/selection/content_complete document metadata 一致；禁止 CF 重解释 raw HTML。text/plain、JSON、PDF 分别保留 `text`、`json`、`pdf_text` 语义，但也必须处于同一 signed v2 envelope。所有模式都先以 response HMAC 认证 audit，再用 audit body SHA-256 和 actual sizes 验证唯一持久化完整 body；article proof excerpt 为 body identity，text/JSON/PDF proof excerpt 为 whitespace-normalized Unicode 前 3000 字符。`signed-body-to-proof-excerpt-v1` derivation、完整 body digest、完整 body 与 excerpt 均绑定 verification HMAC；current 校验确定性重算，不使用当前墙钟。缺失、任意替换 excerpt 或格式错误均失败关闭，不回退 legacy/v1。
