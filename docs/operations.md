...
- migration `033-manual-news-leads.sql` 新增 `manual_news_leads`、`manual_news_evidence`、`manual_news_event_assessments`、`manual_news_lead_audit` 和 date/lineage 单调 `daily_news_review_candidate_generations`，证据行以 `fetch_audit_json` 保存规范化抓取/提取审计；同时给 `daily_news_review_batches` 增加不可变 revision / supersedes / origin / lineage / current / candidate generation 字段及“每日期/lineage 仅一个 active revision”的部分唯一索引。既有日期和 batch 从 generation 0 懒初始化，部署迁移不要求回填业务数据。additive migration `034-manual-news-assessment-verifications.sql` 新增独立 verification 历史表、每 lead/version 最多一个 active 的部分唯一索引及 active/history 查询索引；不得修改或重写已执行的 033。
- `MANUAL_NEWS_VERIFICATION_SECRET` 是每环境独立、至少 32 UTF-8 bytes 的 Wrangler secret。部署前分别执行 `wrangler secret put MANUAL_NEWS_VERIFICATION_SECRET` 和 `wrangler secret put MANUAL_NEWS_VERIFICATION_SECRET --env staging`，不要写入 `wrangler.toml` 或仓库。系统以它对 lead/version、完整最终 assessment 与完整 evidence（包括 `fetch_audit`）做 HMAC-SHA256；API、重放、候选确认均复算 active verification，缺失、过短、旧 policy、错误摘要/HMAC 或 invalidated 一律失败关闭。033 历史 assessment 无 verification 时默认隐藏，需通过 retry 重新生成。
- 正常新线索成本为两次 Pro 调用；有效 active verification 的 Workflow replay 为 0 次模型调用。证据、assessment、verification mutation 都受 expected version + processing owner + status CAS 保护；接管后的旧 Workflow 零写入。证据/策略变化会审计失效旧 verification 并在新版本重新生成；第一次模型调用后、原子保存前的瞬时失败仍是 at-least-once，重试会重新执行两遍模型，不使用第三遍 repair 或代码 fallback。
