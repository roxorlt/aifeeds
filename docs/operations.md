...
- 后续 scheduled/date-scoped 重评分 revision 必须先从当前 active revision 与 durable confirmed leads 合并手工候选，再按 event identity 去重及确定性裁至 10 条；手工候选优先保留 origin/lead provenance。手工候选自身超过上限或发生异常事件碰撞时失败关闭，不能静默丢弃。
