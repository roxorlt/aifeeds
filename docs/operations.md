...
- `manual-news-lead-workflow` 负责安全取证、两次独立的 DeepSeek V4 Pro 严格 JSON 事实核验、事件聚类/跨日去重和评分。第一遍生成 assessment；第二遍逐项覆盖 title、summary、event key/type/time/material update 和每条 claim，只能引用该项获准 evidence，并必须给出在来源 title/excerpt/claims_supported 中连续命中的有界原文 quote。任一事实不支持或确定性 schema/quote 校验失败时 assessment 不暴露并进入 `needs_review`；timeout、429、5xx、D1、研究网关与模型瞬时错误由 Workflow durable step 重试。每次处理持有 owner/attempt/lease，重试耗尽后仅 owner 可 CAS 为 failed；过期的中间状态会重置为 validating 并重新入队。Workflow binding 缺失返回 503，创建实例失败写入可见 failed 状态，绝不回落到请求生命周期。生产与 staging 使用独立 Workflow binding `MANUAL_NEWS_LEAD_WORKFLOW`。
