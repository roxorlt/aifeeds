...
- **Phase 2 — `XTweetPipelineWorkflow`**（worker/src/workflows/x-tweet-pipeline.ts）：每个 instance 5 step pipeline，每步 retry 3 × 10s 指数 backoff，`is_relevant=0` 早退跳过 step 2-4：
  ...
  2. **fan-out (Promise.all)**: `backfill-quote` (syndication，hasQuoteRef 时) + `backfill-reply` (syndication，hasReplyRef 时) + `backfill-retweet` (**SB by-ids 优先 + syndication 兜底**，hasRetweetRef 时，填 `extra.retweet_of` 原作者快照，2026-06-08 改) + `check-longform` (always)
