...
- `MANUAL_NEWS_VERIFICATION_SECRET` 是每环境独立、恰好 64 个小写十六进制字符的 Wrangler secret（建议用密码学安全随机源生成 32 bytes 后编码为 lowercase hex）。部署前分别执行 `wrangler secret put MANUAL_NEWS_VERIFICATION_SECRET` 和 `wrangler secret put MANUAL_NEWS_VERIFICATION_SECRET --env staging`，不要写入 `wrangler.toml` 或仓库。提交与 retry 在入队前检查格式，缺失/非法返回 503；GET 仍可读取 lead，但隐藏 assessment。系统以它对 lead/version、完整最终 assessment（含 `source_atomic_facts_v2` 源语言原子事实、稳定 fact ID、`zh_editorial_projection_v2` 中文编辑投影及逐句映射）、本地确定性派生的双语语义槽（来源归因、认识可能性、计划/未来、进行/完成和 polarity 分组正交签名；参与者/数量范围、对象关系与否定、目标产品后缀、实体、地区、原因、版本及绝对/相对时间），以及 `consumed-semantic-spans-v1` 对象残留门控、完整 evidence（包括 `fetch_audit`）和完整 validated `verification_json` 做 HMAC-SHA256。无法归类的实义对象跨度直接 needs_review，模型返回 supported 也不能绕过。当前验签 policy 为 `fact-evidence-projection-hmac-v8`；同为 v8 但缺少当前双语语义摘要或 v2 双语合同的旧 proof 也会因 digest 不一致而 fail closed。lead API、重放、确认、候选组装、冻结/finalize 与跨日去重统一复算 active verification，密钥轮换、内容篡改、旧 policy/合同、错误摘要/HMAC 或 invalidated 一律视为不存在。033 历史 assessment 及不满足当前合同的 proof 默认隐藏，需通过 retry 重新生成。
