...
| `/admin/dashboard` | GET | 仪表盘默认页：DAU/WAU/MAU 头部 KPI、30 天 DAU 折线、行为漏斗、会话时长直方图、每日新增vs回访（反向口径表格，2026-06-18）、留存矩阵（正向 cohort；未到期格显示「—」非误导性 0%）、事件类型分布（中文标签）、错误明细、重度设备表（留存/回访表默认 5 行可展开）、**🎧 中文配音需求（假门）卡片**。2026-07-23 起 Hero/趋势/漏斗/留存/性能/错误面板复用 `analytics-traffic.ts` 的非真人 UA 口径，历史 crawler/未标记 Headless 行不删除、查询时排除；显式 synthetic 性能 cohort 单列保留。2026-07-24 生产诊断确认 16 路 analytics 首屏并发会把单请求约 0.76–1.4 秒放大到约 10.6–12.6 秒；加载预算规则为 priority `overview` 先完成、其余卡片固定 3 路并发渐进加载（待 `codex/fix-admin-dashboard-loading` 发布）。echarts CDN，单文件 HTML（`worker/src/admin-dashboard.ts`） | CF Access JWT（Basic Auth fallback） |
