...
最后更新：2026-08-13（手工补录行业新闻线索的既有版本已上线 prod：PR #212 合并 + CI 部署，D1 migration 033-036 已执行。Stage-2 explicit key lineage、bounded API profile 与 evidence SQL preflight 仍为本地待部署变更，migration 037 尚未执行；必须先部署并验收 HK client contract，再进入 CF staging/production。见「手工补录行业新闻线索」）
...
| `/admin/dashboard` | GET | 仪表盘默认页：DAU/WAU/MAU 头部 KPI、30 天 DAU 折线、行为漏斗、会话时长直方图、每日新增vs回访（反向口径表格，2026-06-18）、留存矩阵（正向 cohort；未到期格显示「—」非误导性 0%）、事件类型分布（中文标签）、错误明细、重度设备表（留存/回访表默认 5 行可展开）、**🎧 中文配音需求（假门）卡片**。2026-07-23 起 Hero/趋势/漏斗/留存/性能/错误面板复用 `analytics-traffic.ts` 的非真人 UA 口径，历史 crawler/未标记 Headless 行不删除、查询时排除；显式 synthetic 性能 cohort 单列保留。2026-07-24 生产诊断确认 16 路 analytics 首屏并发会把单请求约 0.76–1.4 秒放大到约 10.6–12.6 秒；加载预算规则已随 `209ebc5` 上线：priority `overview` 先完成、其余卡片固定 3 路并发渐进加载。发布后生产真浏览器实测顶部 DAU 1.746 秒出现，16/16 metric 200、最大并发 3、全部 analytics 约 12.669 秒完成。echarts CDN，单文件 HTML（`worker/src/admin-dashboard.ts`） | CF Access JWT（Basic Auth fallback） |
...
| `/r/<key>` | GET/HEAD/Range | R2资源反代。publication物理/legacy namespace在Referer、CORS与R2访问前统一404/no-store；`daily-video/public/<publication>/<role>`仅为head-bound虚拟读取并先全量hash。其它公开资源沿用referer白名单与Range。OPTIONS对private namespace同样deny | 无；publication deny不可覆盖 |
| `/daily/:date` | GET/HEAD | **每日静态日报页**：只从当前授权 unified release head 读取exact private versioned HTML，全量WebCrypto校验并在响应前再次guard；成功`private,no-store`，无/失效head → `noindex` 404 | 无（公开，bot gate豁免；内容fail-closed） |
| `/daily/` `/daily` | GET | 日报归档索引（当前授权release heads按月倒序；裸`daily_pages`不授权）；`/daily/<其它非法段>` → 302本页 | 无（公开，bot gate豁免） |
| `/video/daily/:date` | GET | **独立视频观看页**：只从当前head绑定的published video投影，播放器media key为授权虚拟`/r/daily-video/public/*`；无/失效head或video → `noindex` 404 | 无（公开，bot gate豁免；内容fail-closed） |
...
- 归档索引 / sitemap / llms 均从当前授权 `daily_release_heads` + complete publication graph投影，不做R2 list；`daily_pages`/`daily_videos`及legacy fixed key仅为兼容审计数据
...
**2026-08-27 要闻源与 workflow 自愈加固**：Anthropic 保持 `blog:anthropic` source id/key，发现改为官方 `sitemap.xml` 中的 `/news/<slug>` 页面（不再依赖第三方 RSS bridge）；支持生产 `sitemapindex`，优先文章/news shard并有界下钻最多 12 个子 sitemap。新增 `blog:zai-models`，registry 现为 40 源；通过官方 Hugging Face `zai-org` 模型列表的 Link cursor 最多抓 5 页，以 API 不可变 `_id` 作为 item identity，只认近 30 天有效 `createdAt` 为发布时间；仓库改名、普通 `lastModified` 更新、缺失创建时间及旧 private 仓库转 public 均不制造新事件。该国内官方源的 `.cc` 策略仍为 `deny`。feed item 在 `extra.editorial_type` 持久化来源类型；正式候选、冻结/已应用审核序列、日报页、邮件、日报 API 及 v1/staged Codex 每个对外边界都批量读取完整 item/source/manual provenance，并由同一 formal-news authorization fail-close radar、来源禁用/失配、缺失 backing item 与失效人工证明；`radar` 永远不进入正式候选或对外投递，X 的既有隔离规则不变。UTC 每小时 `:30` 另跑 `blog-workflow-recovery` 与 `podcast-workflow-recovery`：stats、eligible、exhausted、attempts 1–5 CAS 与第 6 次 cause-token CAS 全部复用同一 registry JSON 身份谓词；`source_ref` 必须为 NULL，item `feed_id/feed_key|show_key`、`sources.id/type/ref/config` 必须逐项匹配当前启用 `FEED_REGISTRY`。manual/legacy/non-NULL source_ref、missing/stale/mismatch source 均 fail-closed；唯一跨 kind 兼容是生产已有的 `podcast:<key>:*` + `source_type=blog` + exact `feed_key` text-blog。恢复队列按 attempts、scraped_at、id 公平排序，每小时每源触发最多 50 条且同项小时幂等，最多自动重试 6 次；workflow 真终态清理恢复次数，create 等待期间的晚到失败受 `workflow_completed_at IS NULL` CAS 约束，不能复活 pending。

**可靠 warning outbox（migrations 039 + 041）**：`039-warning-outbox.sql` 的 D1 `warning_outbox` 是耗尽事件 tuple、投递租约、attempts、终态和 retention 的唯一持久权威；共享 KV 数组只保留 UTC 23:00 legacy digest。`041-warning-subject-canonicalization.sql` 增加 per-source `warning_canonical_subjects`、raw alias 映射和冻结高水位 cursor。future attempt `5→6` 通过 `feeds/dedup.ts` 的共享 D1 batch 把 attempts CAS、NFC canonical subject、exact raw alias 和 cause token 因果绑定；CAS lost/terminal wins 不留下授权映射，alias/canonical 任一缺口使 readiness fail-closed。历史 materializer 在每小时 `:15` 以 blog/podcast 两个独立 `waitUntil` action 运行：每源真实执行最多 4 页、每页 50 个 raw row，每页处理后以 owner/lease/cycle/high-water/old-after 全字段 CAS 推进 cursor；exact 已映射 alias 在 eligibility 中排除，trailing ineligible 仍推进 raw cursor。短页或触达 frozen high-water 时原子把 `after_item_rowid` 归零、`cycle_no+1`、刷新下一周期 frozen high-water并返回 `wrapped=true`；crash 后新 owner 从最后成功页继续，旧 owner 无法推进。单次上限仍为 200 raw rows，401/4001 byte-distinct aliases在当前及后续周期都分别以 3/21 次调用有界推进，不依赖 SQLite Unicode 比较。

切换 gate：P=`WARNING_OUTBOX_PRODUCER_ENABLED`、D=`WARNING_OUTBOX_DRAIN_ENABLED`、B=`WARNING_CANONICAL_BACKFILL_ENABLED`、R=该 source durable readiness。安装顺序固定为 039→041→含 future hook 的代码→B=1 暖机；D 必须先于 P。`P=0,D=0` 由 legacy KV 唯一负责；`P=0,D=1` 使用 D1 canonical tuple reservation 后才调用 legacy KV；`P=1,D=1,R=0` 仍由同一 bridge 唯一负责；只有 `P=1,D=1,R=1` 才由 D1 producer 唯一负责。`P=1,D!=1` 是 invalid configuration，action 失败且不猜测回退。回滚先关 P、保持 D 排空既有 D1 backlog；进入新 UTC period 且 backlog 已终态后才可关 D。不得 drop 041 或重置 readiness/cursor，不得 D1/KV 双写。

bridge 在 legacy KV 查询前原子插入 `(workflow_retry_exhausted, source_type, NFC subject, UTC day)` 的 `PRODUCER_LEGACY_OWNED` reservation；唯一键冲突抑制 legacy。D1 查询/插入失败时 fail-closed，不调用 KV；KV 明确未入队才释放首轮 reservation供同日重试。进程在 KV 成功后结果未知的窗口，30 分钟后只允许将同 tuple 提升为 D1 deliverable，不再二次写 KV；因此每 canonical event/period 的不可避免重复上界为 1，并记 `bridge_duplicate_possible`。

D1 producer 只扫描 041 已映射且当前仍为 registry-managed、未删除、非 manual、未完成、attempts≥6 的 canonical subject；当日 legacy marker/tuple 在 SQL LIMIT 前排除。永久输入/完整性拒绝写 durable quarantine，瞬态 D1 错误保持可重试。drain 在现有 `*/5` 每个 tick 独立运行：due lookahead每批101、最多2批、最多claim/send 200 rows；claim 原子增加 attempts且 `attempts<6`，到6不再有第七次发送。chunk按 attempts 分桶，每块最多25 events/16384 UTF-8 bytes；至少一个 PushDeer destination 明确成功才 ack，HTTP/provider失败回 pending，exception或ack未知保持 lease供至少一次重试。UTC 03:35 retention独立看501、最多删除500个到期终态。`warning-outbox-drain`、`warning-outbox-retention`、`warning-digest` 与两条 canonical backfill在 cron_runs/任务看板中均为独立 action，任一失败不吞同分钟兄弟 action。

required warning cron observation 使用专用 canonical serializer，UTF-8硬上限3840 bytes且不存 raw subject/title/error detail。稳定字段包括 gate/table/status/error、due/lookahead/cap、claim/lease conflict、pre/post-send unresolved、delivered/retried/failed-at-limit/stale-at-limit、chunk与destination守恒、retention eligible/attempt/delete/conflict，以及 `integrity_conflict_count/digest/sample_tokens`、`quarantine_conflict_count/digest`、`alert_producer_quarantine_conflicts`、`alert_legacy_owned`、`alert_bridge_suppressed`、`bridge_duplicate_possible`。sample最多4个16-hex token；oversize返回固定 `CRON_RESULT_OVERSIZE` 并使 required action失败，禁止字符串截断或持久化 `integrity_conflict_ids`。

**Append-only daily release（migration 040）**：`040-daily-release-publications.sql` 新增3 TiB全局累计运营预算 singleton、per-date page 16/video 4 reservation slots、publication/object/manifest rows及统一 `daily_release_heads`。migration后预算保持 `uninitialized`；必须先离线盘点 private namespace，把 `legacy_baseline_bytes/object_count/inventory_digest` 与审计记录写入，再设 `active`。随后按 `DAILY_PUBLICATION_RESERVATION_ENABLED`→`DAILY_PUBLICATION_PUT_ENABLED`→`DAILY_PUBLICATION_PROMOTION_ENABLED` 顺序开 gate；任何依赖 gate 未开或逆序组合都 fail-closed，不能恢复 legacy fixed-key writer。reservation trigger在一个 statement-error-backed D1 batch中同时占用 exact slot与全局累计 bytes；same business revision replay复用256-bit attempt key/token且不重复占用，失败 reservation 永久占 slot/bytes。70/85/95%发容量预警，100%原子拒绝；预算扩容只允许独立审计变更且不得低于已占用。

**Publication capacity warning outbox（migration 042）**：042必须在040之后、inventory activation之前安装；它以独立control singleton、永久70/85/95 crossing和`publication_capacity_warning_outbox`持久化容量告警，不复用或改写039。budget increase 的 unknown outcome/replay 必须匹配 immutable 完整 audit command tuple（audit id、新预算、actor、reason、ticket、created-at及结构约束）；若当前 budget/control 是该 audit 之后仅由合法 040 reservations 形成的 descendant，可 authoritative replay 且不新增 audit/crossing/epoch，后续另一扩容、freeze或任一 tuple 字段变化均 fail-closed。`PUBLICATION_CAPACITY_WARNING_PRODUCER_ENABLED=1`每5分钟物化crossing，`PUBLICATION_CAPACITY_WARNING_DRAIN_ENABLED=1`每5分钟独立租约投递，UTC 03:35清理已投递30天/失败90天的042 outbox；crossing永不删除。启用顺序为042→inventory audit/activation→capacity drain→capacity producer→publication R/P/H。回滚publication writer时关闭R/P/H和capacity producer，但保持capacity drain排空既有pending；禁止写legacy KV、禁止双写039、禁止删除crossing或回收budget bytes。cron结果必须小于3840 UTF-8 bytes；`partial/error`或`cron_runs`写失败均视为action失败。

publication physical objects只允许 private `daily/versions/**` 与 `daily-video/candidates/**`，业务 adapter只暴露 HEAD/PUT，DELETE调用恒为0；unknown/late PUT用同一key/同一bytes幂等重试。`/r/*` 对物理/legacy namespace的GET/HEAD/Range/OPTIONS在CORS和R2读取前统一404/no-store，host、Referer、encoded path不能放宽；`daily-video/public/<video-publication-id>/<role>`仅是虚拟 outward key，先以当前 release head授权，再读取并全量校验private object。page/video统一head支持 `none`、`reuse_current`、`joint_new`；D1 head CAS是唯一发布线性化点，不宣称D1/R2跨系统原子。page 2 MiB、MP4 64 MiB、poster 8 MiB、VTT 1 MiB；最坏324 MiB/日期、28对象，3 TiB在baseline=0时约26.5274个最坏闰年（保留10%运营余量约23.8747年）。所有 reservation、PUT、promotion、outward 边界都从实际bytes经Worker/WebCrypto重算SHA/size/manifest；SQLite只断言格式与跨表exact关系，不使用SHA UDF。

`daily_pages`/`daily_videos`仅为compatibility/audit projection。`/daily/:date`、归档、watch/media、日报/视频 sitemap、IndexNow目标、相邻日期与freshness monitor只承认当前完整、已发布、重新通过formal-news/batch/manual guard的统一 release head；archive/sitemap 先完成 page+video graph 等异步读取，再把 shared joined guard 作为该 release 的最后授权读取。`daily_pages` compatibility UPSERT 本身使用同一 joined guard 的单条受权写入；前日补链后、page/video IndexNow HTTP 前以及 upload success 前再 authoritative reread，失败不发 IndexNow。读取物理bytes后在返回前再次guard，成功响应仍为`private, no-store`。feature gate关闭只停止新reservation/PUT/promotion，不恢复固定key公开读取、R2 DELETE或裸compatibility表授权。
...
- **21 个表**：
  ...
  - `daily_pages`（2026-07-06 每日静态日报页 SEO P0 新增，migration 025；2026-08-27 降级）— 每日静态日报页 compatibility/audit projection。migration 040 上线后，任何 outward page/archive/sitemap/freshness 结果都必须从当前授权 `daily_release_heads` 及其完整 publication graph投影；裸 `daily_pages` 行和 legacy `daily/YYYY-MM-DD.html` 不能授权公开内容。projection只在exact head/generation/publication digest仍匹配时更新。
  - `publication_storage_budget` / `publication_reservations` / `append_only_publications` / `append_only_publication_objects` / `publication_manifest_commits` / `daily_release_heads`（migration 040）— append-only daily page/video 的全局累计预算、per-date slot、immutable object tuple、manifest和唯一公开head。物理R2对象永远private且业务运行时不DELETE；outward reader从head解析exact key并全量WebCrypto校验。
  - `warning_outbox`（migration 039）— workflow retry exhausted可靠投递、租约/attempts/终态/retention权威表。
  - `publication_capacity_warning_control` / `publication_capacity_threshold_crossings` / `publication_capacity_warning_outbox`（migration 042）— publication累计容量snapshot、永久70/85/95 crossing和独立可靠投递状态；不复用039或legacy KV。
  - `warning_canonical_subjects` / `warning_subject_aliases` / `warning_subject_scan_cursors`（migration 041）— NFC canonical warning identity、raw alias与per-source冻结高水位/readiness；D1 producer只在对应source ready时取得authority。
...
- 内部 API（`Bearer DAILY_NEWS_REVIEW_SECRET`）：
  ...
  - `GET /api/digest/daily-news-leads?date=YYYY-MM-DD`：最多返回 50 条 bounded summary（含 `response_profile=manual_news_leads_summary_v1`、`schema_version=1`、`evidence_count`，不含 evidence/audit/excerpt）；`GET /api/digest/daily-news-leads/:id`：按需返回单条 `manual_news_lead_detail_v1` detail，最多 8 条、每条最多 3000 Unicode code points / 12000 UTF-8 bytes 的 excerpt，不返回完整正文或 fetch audit。所有 mutation 的 `lead` 也使用同一 detail DTO。HK 消费契约固定在 `workflows/aifeeds-daily/fixtures/manual-news-leads-api-v1-contract.json`，工作台必须先 list，再按需 GET detail。
  - `POST /api/digest/daily-news-leads/:id/retry` 与 `/confirm-candidate`：必须同时带幂等键和 `expected_version`。
- `manual-news-lead-workflow` 负责安全取证、两次独立的 DeepSeek V4 Pro 严格 JSON 事实核验、事件聚类/跨日去重和评分。第一遍生成源语言原子事实及逐句 fact-ID 映射的严肃中文 title/summary 投影；第二遍先逐项核验源事实、event key/type/time/material update，只能引用该项获准 evidence 并给出在来源 title/excerpt/claims_supported 中连续命中的有界原文 quote，再独立核验每个中文投影与已验证源事实语义等价且无事实扩张/遗漏。任一事实或投影不支持、确定性 schema/slot/quote 校验失败时 assessment 不暴露并进入 `needs_review`；timeout、429、5xx、D1、研究网关与模型瞬时错误由 Workflow durable step 重试。每次处理持有 owner/attempt/lease，重试耗尽后仅 owner 可 CAS 为 failed；过期的中间状态会重置为 validating 并重新入队。Workflow binding 缺失返回 503，创建实例失败写入可见 failed 状态，绝不回落到请求生命周期。生产与 staging 使用独立 Workflow binding `MANUAL_NEWS_LEAD_WORKFLOW`。
- migration `033-manual-news-leads.sql` 新增 `manual_news_leads`、`manual_news_evidence`、`manual_news_event_assessments`、`manual_news_lead_audit` 和 date/lineage 单调 `daily_news_review_candidate_generations`，证据行以 `fetch_audit_json` 保存规范化抓取/提取审计；同时给 `daily_news_review_batches` 增加不可变 revision / supersedes / origin / lineage / current / candidate generation 字段及“每日期/lineage 仅一个 active revision”的部分唯一索引。既有日期和 batch 从 generation 0 懒初始化，部署迁移不要求回填业务数据。additive migration `034-manual-news-assessment-verifications.sql` 新增独立 verification 历史表；migration `037-manual-news-proof-key-ids.sql` 分别给 evidence/proof 增加 `response_key_id`/`verification_key_id`。legacy 空 ID 不原地迁移，只按 unknown key fail-hidden。不得修改或重写已执行的 033-036。
- 每环境必须成对配置当前 proof key：`MANUAL_NEWS_VERIFICATION_KEY_ID` 与 64 位 lowercase-hex `MANUAL_NEWS_VERIFICATION_SECRET`；轮换时可用 secret `MANUAL_NEWS_VERIFICATION_KEYRING_JSON` 保留最多 8 个精确 `{id,secret}` 历史项。ID 为 1-64 位 lowercase ASCII `a-z0-9._-`，current+history 的 ID 与 secret 均不得重复。提交与 retry 在入队前严格校验；配置缺失/畸形或 persisted key ID 未配置属于 dependency unavailable：GET 隐藏 assessment，且不得修改 verification、published item 或 audit。只有 ID 已知、schema 有效但 HMAC 不匹配才属于 tamper 并触发原子 quarantine。proof canonical payload 绑定 `verification_key_id` 和每条 evidence 的 `response_key_id`；历史加载只按 persisted ID 选择 exact key，不尝试任意 key。系统以选定 proof key 对 lead/version、完整最终 assessment（含 `source_atomic_facts_v2` 源语言原子事实、稳定 fact ID、`zh_editorial_projection_v2` 中文编辑投影及逐句映射）、本地确定性派生的双语语义槽、完整 evidence、规范化 signed-v2 provenance 和完整 validated `verification_json` 做 HMAC-SHA256。当前验签 policy 为 `fact-evidence-projection-hmac-v10`；v9、旧 policy、legacy/v1/unsigned/malformed evidence、错误摘要/HMAC 或 invalidated proof 一律 fail closed，经现有 invalid-proof 路径隐藏并隔离，必须显式 retry/re-evidence；绝不原地迁移或升级 proof。lead API、重放、持久化加载、确认、候选重建、冻结/finalize 与跨日 prior-event 加载统一复算同一 current-proof 条件。已持久化 evidence 的检查不使用当前墙钟重新判定采集新鲜度。
- 正常新线索成本为两次 Pro 调用；同一次 Workflow 的有效 active verification replay 为 0 次模型调用。运营侧显式 retry 会在同一 D1 batch 中审计并失效旧 active verification，保证下一轮真正重新检索和生成。证据、assessment、verification mutation 都受 expected version + processing owner + `processing_attempt` fencing token + status CAS 保护；接管后即使 owner 字符串相同，旧 attempt 也零写入。证据 replace、verification create/invalidate 均写带 owner/attempt/version/digest/nonce 的审计。证据/策略变化会审计失效旧 verification 并在新 attempt/version 重新生成；第一次模型调用后、原子保存前的瞬时失败仍是 at-least-once，重试会重新执行两遍模型，不使用第三遍 repair 或代码 fallback。
- 确认线索只产生候选池 revision：冻结前进入当天首批候选，冻结后生成 V2+ 并标记旧批次被替代。无 active batch 的确认写入以“仍不存在 active date/lineage batch”为 item、lead、audit 和 generation 递增的共同条件；若首次 freeze 抢先则整批不留部分确认并返回 revision conflict。反向时序中，scheduled freeze 必须先快照 generation、再采集候选，并在 revision 插入时 CAS 同一个值；中途 pre-freeze confirm 提交会令旧插入失败，freeze 重采集 durable leads 后才可发布。它不会修改已发布 Top 5，也不会触发 HK 重渲染；仍须用户在原审核区选择 1–5 条、排序并显式重新生成。
- 后续 scheduled/date-scoped 重评分 revision 必须先从当前 active revision 与 durable confirmed leads 合并手工候选，再按 event identity 去重及确定性裁至 10 条；已发布选择与全部手工候选均不可淘汰，只能从未选择的 scheduled 尾部确定性淘汰。不可淘汰集合超过上限时以 `candidate_cap_exhausted` 失败关闭，整批不写且线索不确认，不能静默丢弃 provenance。
- Worker 不直接 fetch 用户域名；URL 取证和开放网络搜索只调用 `MANUAL_NEWS_RESEARCH_ORIGIN` 固定 HTTPS 研究网关，并使用 secret `MANUAL_NEWS_RESEARCH_TOKEN`。每环境 response key inventory 为当前 `MANUAL_NEWS_RESEARCH_RESPONSE_KEY_ID` + `MANUAL_NEWS_RESEARCH_RESPONSE_SECRET`，轮换时可用 secret `MANUAL_NEWS_RESEARCH_RESPONSE_KEYRING_JSON` 保留最多 8 个历史 `{id,secret}`；当前 secret 与对应 HK `AIFEEDS_MANUAL_NEWS_RESPONSE_SECRET` 使用同一 32-byte lowercase-hex key。Worker 先让 response HMAC 在配置 keyring 中唯一匹配，再持久化 matched `response_key_id`。正式 `/v1/document` 请求显式携带 `extraction_mode=article_text_v2`、`response_profile=proof_excerpt_v1`、随机 nonce、canonical request timestamp、limits 与 max redirects；缺失或未知 profile 一律失败关闭，旧 v2 兼容仅由 HK 承担。响应 audit 必须完整包含 `protocol_version=article_text_v2`、相同 nonce/timestamp、canonical extracted timestamp、逐跳 peer audit、final URL、source/extraction、requested/applied/actual limits、无截断标记、成功 parser、完整 body SHA-256、`response_hmac_contract=hmac-sha256-canonical-json-all-fields-except-response_hmac-v1`、严格 `proof_excerpt` claims（canonical cap 字段仅为 `max_code_points=3000`；legacy `max`、错误值或双字段均拒绝）及对除 `response_hmac` 外完整 canonical audit 的 HMAC。所有模式都先认证 response HMAC，再验证完整 transport body 后确定性派生 excerpt 并立即解除完整 body 引用；后续 current 校验不使用当前墙钟。完整 body 不得进入 evidence、claims、D1、Workflow step output、API DTO、日志/错误 metadata 或 proof canonical payload。persisted load 在 JSON materialize/parse 前先用 aggregate SQL 检查 count 与逐列最大长度，materialize 显式列且 `LIMIT 9`；超限通过 active verification snapshot 原子 quarantine，batch 任一步失败整体 rollback。
- 发布顺序（硬门禁）：先安装并验证 HK dedicated Chromium sandbox/account/resource gate、研究网关、current response key ID/secret 与历史 keyring，并完成 authenticated gateway 及 legacy/v1/v2/tamper/stale/private-target smoke；同时让 HK client 通过共享 `manual_news_leads_api_v1` fixture 的旧 full-list/current response 与新 summary→detail contract 测试并部署，先在 production 对尚未变更的 CF 验证旧 current response。以上全部通过后，才按顺序在 CF staging 执行 migration 037，配置两组 current `KEY_ID`/secret 与可选 keyring、部署 Worker/Workflow 并做端到端 smoke；最后进入 CF production 并验证带 `manual_news_leads_summary_v1` marker 的新 summary→detail response。禁止 CF consumer-first 发布，也不得让 unprofiled HK client 静默解释 summary 为旧 full-list。
...
- **origin（人审优先，2026-08-19）**：staged payload 顶层新增 `origin: 'review' | 'auto'`。`review` = 这批内容里的行业要闻序列出自人工审核提交（含「继承人审序列、仅自动剔除失效条目」的版本），`auto` = 当日尚无人审的纯自动排序。origin 描述**内容**而非触发者：08:00 cron 的 finalize 若带的是人审序列同样标 `review`；`foundation`/`papers` 不含 news 板块，恒为 `auto`。人审提交路径（`POST /api/digest/daily-news-review`）显式传 `review`，其余路径按当日审核批次的 `human_reviewed` 标记推断。该字段不参与 `content_hash`/`render_key`，HK 可安全用它拒绝「自动推送覆盖人审结果」。旧版本 HK 收到未知字段应忽略，不影响 v2 解析。
- **人审优先的 D1 保证**：migration `038-news-review-human-priority.sql` 给 `daily_news_review_batches` 增加 `human_reviewed`。人工提交（`submitNewsReviewSelection`）置 1；scheduled freeze、sanitize、手工线索 confirm 三条建新版本的路径都继承该标记，并把人审序列继续写进新版本的 `applied_selected_ids`（只剔除已失效 id，其余相对顺序不变，自动补位追加在末尾）。因此 `daily-digest-rescore` / cron 重算只会刷新候选池与 `default_selected_ids`，不再把生产选择回落到自动排序（2026-08-19 人审 r2 被自动 r3 覆盖事故）。**部署顺序：先跑 migration 038（staging → prod），再部署 Worker**；反过来会让冻结/提交语句撞 `no such column: human_reviewed`。
...
> 早 8 点 `digest-node-run-workflow` 算完当天榜单后，Phase 4 **非阻塞**生成 SEO 静态日报页（append-only private object + guarded release-head promotion + compatibility projection + IndexNow ping），失败只记日志。伺服路由见上方 §1；2026-08-27后的权威发布合同见 `docs/plans/2026-08-27-news-pool-hardening-architecture.md` v4/v5。
...
- **产物**（`digest/daily-page-run.ts generateDailyPage`）：① 040 reservation/publication/object/manifest graph（same business revision复用同一256-bit attempt key）；② R2 private `daily/versions/<attempt>/page.html` HEAD/PUT；③ full-byte验证后的current formal/batch guard + D1 release-head generation CAS；④ exact head保护下更新`daily_pages`兼容projection；⑤ 前一**有效release日期**重渲染补链；⑥ IndexNow ping。任一步失败都不改变公开head，R2孤儿永久private且业务运行时不DELETE。
...
- 🆕 **缺页 / 失败告警（2026-07-07，两道防线，`digest/daily-page-monitor.ts`）**：
  ...
  - **② 缺页兜底检查**（`checkDailyPageFreshness`，挂在 `scheduled()` 的 **UTC 01:00** tick，仅 `DAILY_PAGE_ENABLED==='1'` 时启用，独立 `waitUntil`）：读取今天（BJT date）通过当前formal/batch guard的完整release head，其`promoted_at_ms`必须晚于今天 UTC 0 点；**无有效head / 陈旧head / 授权已失效** → PushDeer「[SEO] 今日日报页未生成」。裸`daily_pages.generated_at`不计成功。**防重**：KV key `DAILY_PAGE_MISSING_ALERTED_<BJT-date>`（TTL 25h）保证当天只告一次。
...
| 路由 | 伺服 | 缓存头 |
|------|------|--------|
| `/daily/:date` | 当前 unified release head → private versioned HTML全量hash+最终授权guard；无/失效head → noindex 404 | `private, no-store` |
| `/daily/` `/daily` | 当前授权 release heads 的归档投影（按月倒序；裸 `daily_pages` 不授权） | `public, max-age=3600` |
| `/video/daily/:date` | 当前head绑定的published video动态渲染；无/失效head或video → noindex 404 | `private, no-store` |
...
| `/video-sitemap.xml` | 只投影当前授权head；landing=`/video/daily/:date`，媒体/封面为head-bound虚拟`API_BASE/r/daily-video/public/*` | `public, max-age=3600` |
...
- **`daily_pages` / `daily_videos`**：保留为compatibility/audit projection，不能授权页面、视频、归档或sitemap。
- **`publication_storage_budget` / `publication_reservations` / `append_only_publications` / `append_only_publication_objects` / `publication_manifest_commits` / `daily_release_heads`**（migration 040）：append-only daily publication预算、slot、对象graph与唯一公开head；见上方「Append-only daily release」。
...
不要删除 state 或手工改写 `public/current`。

2026-07-21 首期生产记录：Worker 内容策略 v5 对 TechCrunch AI、The Verge AI、MIT
Technology Review AI 分批审核 219 条，最终 137 条 live、82 条 deny；13 个曾短暂公开后
撤回的 URL 均返回 404。VPS state 为 `last_seq=163`、`bootstrap=null`、137 个 live 页面，
`aifeeds-cc-sync.timer` 为 `active/enabled`。`sitemap-cn-0001.xml` 含 137 个内容页与 3 个归档页，
`sitemap-static.xml` 含 8 个固定页，去重后共 148 个可发现 URL；全部直接 200。137 个内容页
逐页通过 200、精确自 canonical、无 noindex/自动跳转、`.com` CTA UTM 和 v5 风险词门禁，
另做固定 50 条标题/摘要人工复核。当前国内稳定索引只有 `0001` 一片；四家最终提交清单、
文件 SHA-256 与逐平台步骤见
[`docs/cc-search-engine-submission-runbook.md`](cc-search-engine-submission-runbook.md)。生产
`CC_MIRROR_ENABLED` 仍保持关闭，在扩大来源或继续历史回填前必须重复分批审核与抽样门禁。
