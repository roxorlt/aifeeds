...
当前审核策略版本为 **v3**。系统审核的文本与页面实际可见文本采用同一渲染口径，长内容只取稳定的头部、中段和尾部样本，最多 11,000 字符。
...
v2 在模型前增加了保守的确定性词法 backstop；v3 根据生产抽样继续覆盖监管机构、立法、议员、警方和执法机关。明确涉及政治治理、军事冲突或对华负面定性的内容直接拦截；仅明确涉及制裁/出口管制的内容进入人工复核。企业内部的数据治理、模型治理或产品管理不因“治理”一词被直接拦截。词法未命中不代表自动通过，后续仍必须经过结构化模型审核。
