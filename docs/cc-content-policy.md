...
当前审核策略版本为 **v5**。系统审核的文本与页面实际可见文本采用同一渲染口径，长内容只取稳定的头部、中段和尾部样本，最多 11,000 字符。
...
v2 在模型前增加了保守的确定性词法 backstop；v3 根据生产抽样继续覆盖监管机构、立法、议员、警方和执法机关；v4 将针对中国、中国企业或中国用户的限制、禁用、封禁与阻断纳入确定性对华风险，同时区分“中国公司限制自己的产品用户”等反向施事语境；v5 精准覆盖检察官、陪审团等刑事司法语境，以及儿童性虐待材料、谋杀、纵火和重罪等严重伤害内容，但不因普通 AI 版权诉讼或虚构城市招聘实验自动拦截。明确涉及政治治理、军事冲突、严重伤害或对华负面定性的内容直接拦截；仅明确涉及制裁/出口管制的内容进入人工复核。企业内部的数据治理、模型治理或产品管理不因“治理”一词被直接拦截。词法未命中不代表自动通过，后续仍必须经过结构化模型审核。
