...
// 2026-09-07 起三个阶段整体提前两小时:foundation 04:30 / editorial 05:50 / papers+finalize 06:00。
const AT_0600 = bjt(DATE, 6, 0);   // editorial 已过 due(05:50)+grace(10)
const AT_0550 = bjt(DATE, 5, 50);  // editorial 刚到点,还在宽限期内
...
describe('窗口与开关', () => {
  test('staged 模式关闭时直接跳过,不读 D1', async () => {
    const result = await checkStagedDailyStages(
      makeEnv({ DAILY_STAGED_PUSH_ENABLED: '0' }), AT_0600,
    );
  ...
  test('阶段计划时刻 + 宽限期未到时不查也不告警', async () => {
    const result = await checkStagedDailyStages(makeEnv(), AT_0550);
    // 05:50 时 foundation(04:30+15)已到期,editorial(05:50+10)还没到。
    expect(result.checks.map((check) => check.stage)).toEqual(['foundation']);
  });

  test('窗口尾巴过去后不再反复查同一天', async () => {
    // BJT 12:00 距 finalize(06:25)已超过 240 分钟尾巴。
    const result = await checkStagedDailyStages(makeEnv(), bjt(DATE, 12, 0));
...
describe('缺失检测与告警', () => {
  test('editorial 快照缺失(9/2 的形状)→ BJT 06:00 就告警', async () => {
    ...
    const result = await checkStagedDailyStages(makeEnv(), AT_0600);
    ...
    expect(String(call[3])).toContain('BJT 05:50');
  });
  ...
  test('DAILY_PUSH_ENABLED 未开时不因缺 pushed_at 误告警', async () => {
    ...
    const result = await checkStagedDailyStages(makeEnv({ DAILY_PUSH_ENABLED: '0' }), AT_0600);
  ...
  test('同一 (日期, 阶段) 每天只告一次', async () => {
    ...
    const first = await checkStagedDailyStages(env, AT_0600);
    const second = await checkStagedDailyStages(env, AT_0600 + 5 * 60_000);
  ...
  test('KV 故障只影响去重,不吞掉告警本身', async () => {
    ...
    const result = await checkStagedDailyStages(env, AT_0600);
  ...
  test('全部阶段健康时静默', async () => {
    ...
    const result = await checkStagedDailyStages(makeEnv(), bjt(DATE, 6, 30));
  ...
  test('9/2 重演:仅凭 D1 状态就能在 BJT 06:00 报出来,不需要 workflow 侧跑过任何代码', async () => {
    // 事故形状:editorial 与 papers 两次 workflow 都没能留下 editorial 快照。
    ...
    expect(result.alerted).toBe(1);
    // editorial 计划时刻 + 10 分钟宽限就报;当天邮件 08:00 才发,留足了人工补救的时间。
    expect(result.checks.find((check) => check.stage === 'editorial')?.reason).toBe('missing_snapshot');
  });

  test('变异验证:把 editorial 的宽限期放宽到 30 分钟,BJT 06:00 这一刻就报不出来了', async () => {
    vi.mocked(getDailyStageState).mockResolvedValue(null);
    const editorial = STAGED_STAGE_DEADLINES.find((entry) => entry.stage === 'editorial')!;
    // 当前配置:05:50 + 10 分钟宽限 → 06:00 整点这一 tick 恰好开始检查。
    expect(editorial.dueMinuteBjt + editorial.graceMinutes).toBe(6 * 60);
    const at0600 = await checkStagedDailyStages(makeEnv(), AT_0600);
    expect(at0600.checks.map((check) => check.stage)).toContain('editorial');

    // 变异体:宽限期若放宽到 30 分钟(= 06:20 才检查),06:00 这一刻 editorial 不在检查范围内,
    // 告警要推迟 20 分钟 —— 断言据此必红。
    const mutatedFrom = editorial.dueMinuteBjt + 30;
    const minuteAt0600 = 6 * 60;
    expect(minuteAt0600 >= mutatedFrom).toBe(false);
  });
});
