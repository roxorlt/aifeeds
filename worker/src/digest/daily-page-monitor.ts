...
//   2. checkDailyPageFreshness —— 缺页兜底检查:较晚 tick(UTC 01:00)查 D1 daily_pages 今天(BJT)
//      那行的 generated_at 是否晚于今天 UTC 0 点(=今天自然跑真生成了);无行 / 陈旧行 → 告警
...
export interface DailyPageFreshnessResult {
  ...
  generatedAt: string | null;     // daily_pages 该行 generated_at(无行则 null)
  alerted: boolean;               // 本次是否发了告警
  reason: 'fresh' | 'missing' | 'stale' | 'already_alerted';
}

// 缺页兜底检查。晚于自然跑(UTC 0 点)的 tick 调用(建议 UTC 01:00)。
// 今天(BJT date)那行 generated_at >= 今天 UTC 0 点 → 视为今天自然跑真生成了 → 静默;
// 无行(missing)/ generated_at 早于今天 UTC 0 点(stale)→ 发告警,KV 标记当天只告一次。
export async function checkDailyPageFreshness(env: Env): Promise<DailyPageFreshnessResult> {
  const date = bjtDateStr();
  const now = new Date();
  // 今天 UTC 0 点(墙钟午夜)的 epoch ms。generated_at 由 persistPage 写 new Date().toISOString()(UTC)。
  const utcMidnight = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate());

  const row = await env.DB.prepare(
    `SELECT generated_at FROM daily_pages WHERE date = ?`,
  )
    .bind(date)
  ...
  await pushDeerAlert(
    ...
    [
      ...
      generatedAt
        ? `- daily_pages 该行 generated_at=\`${generatedAt}\`,早于今天 UTC 0 点(疑似陈旧行,未刷新)`
        : `- daily_pages 无 \`${date}\` 行(Phase 4 未成功执行)`,
      '',
      '排查:node-run 早 8 点 Phase 4 是否抛错 / `DAILY_PAGE_ENABLED` 开关 / 选品是否为空 / R2·D1 写入是否失败',
    ].join('\n'),
  );
